                    ...
                    <tbody class="text-gray-400">
                        @foreach ($orders as $order)
                            <tr class="border-b hover:bg-gray-800 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <span
                                        class="px-3 py-1 rounded-full text-white {{ $order->order_status === 'completed' ? 'bg-green-500' : 'bg-yellow-500' }}">
                                        {{ ucfirst($order->order_status) }}
                                    </span>
                                </td>
                                <td class="px-6 py-4">
                                    <span
                                        class="px-3 py-1 rounded-full text-white {{ $order->payment_status === 'paid' ? 'bg-green-500' : 'bg-red-500' }}">
                                ...
                                <td class="px-6 py-4">
                                    <a href="{{ route('user.orders.details', $order) }}"
