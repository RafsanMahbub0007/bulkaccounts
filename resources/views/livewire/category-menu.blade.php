<div x-data="{
    ...
    @mouseenter="if (window.innerWidth > 1024) { clearTimeout(timeout); open = true; }"
    @mouseleave="if (window.innerWidth > 1024) { timeout = setTimeout(() => open = false, 150); }"
    class="relative">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-5 gap-4">
            ...
            <nav class="col-span-1 bg-gray-900/60 rounded-lg p-3 max-h-[60vh] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-700">
                @foreach ($this->filteredCategories as $index => $category)
                    <button
                        @mouseenter="activeIndex = {{ $index }}; $wire.set('activeIndex', {{ $index }})"
